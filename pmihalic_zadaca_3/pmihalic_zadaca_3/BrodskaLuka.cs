...
public class BrodskaLukaMain
{
    ...
    public static Collection kolekcijaMolova = new Collection();
    public static Collection kolekcijaVezova = new Collection();
    ...
    static void Main(string[] args)
    {
        ...
        foreach (Mol mol in molovi)
        {
            int i = 0;
            int j = 0;
            luka.kolekcijaMolova[i] = mol;
            i++;
            foreach (MolVez molVez in moloviVezovi)
            {
                if (molVez.IdMol == mol.IdMol)
                {
                    foreach (int idVez in molVez.IdVezovi)
                    {
                        foreach (Vez vez in vezovi)
                        {
                            if (vez.Id == idVez) {
                                mol.kolekcijaVezova[j] = vez;
                                j++;
                            }
                        }
                    }
        ...
        luka.PopuniKolekcijuVezovaPremaMolovima();

        kolekcijaMolova = luka.kolekcijaMolova;
        kolekcijaVezova = luka.kolekcijaVezova;
    ...
    private static void PrikaziIzbornik()
    {
        ...
        Console.WriteLine("     | spremanje stanja vezova i vremena:   SPS \"[naziv]\"                                                 |");
        Console.WriteLine("     | vracanje stanja vezova i vremena:    VPS \"[naziv]\"                                                 |");
        ...
        switch (oznaka)
        {
            ...
            case "RP":
                ...
                break;
            case "SPS":
                if (Regex.Match(naredba.Trim(), "^SPS \".+\"$").Success)
                {
                    SpremiStanje(naredba.Substring(naredba.IndexOf(" ") + 1).Trim());
                }
                else Console.WriteLine("Krivi format argumenata za naredbu SPS");
                PrikaziIzbornik();
                break;
            case "VPS":
                if (Regex.Match(naredba.Trim(), "^VPS \".+\"$").Success)
                {
                    VratiStanje(naredba.Substring(naredba.IndexOf(" ") + 1).Trim());
                }
                else Console.WriteLine("Krivi format argumenata za naredbu VPS");
                PrikaziIzbornik();
                break;
            case "Q":
    ...
    private static void PregledajStatusVezova()
    {
        ...
        {
            Vez vez = (Vez)iterator.CurrentItem();
            if (raspored.Count == 0)
            {
            ...
            } else if (raspored.Count > 0)
            {
                ...
                if (zauzet) Console.WriteLine(String.Format(formatPodataka, vez.Id, vez.Oznaka_veza, vez.Vrsta, vez.Stanje, ++redni_broj + "."));
                else Console.WriteLine(String.Format(formatPodataka, vez.Id, vez.Oznaka_veza, vez.Vrsta, vez.Stanje, ++redni_broj + "."));
            }
            iterator.Next();
        }
    ...
    private static Collection ProvjeriStanjeVezova()
    {
        Iterator iterator = new Iterator(kolekcijaVezova);
    ...
    private static List<string> SlobodniVezovi(string argumenti)
    {
        ...
        while (!iterator.IsDone())
        {
            Vez v = (Vez)iterator.CurrentItem();
            zauzet = false;
    ...
    private static void SpremiStanje(string naziv)
    {
        Console.WriteLine(virtualnoVrijeme);
        originator.Vezovi = ProvjeriStanjeVezova();
        originator.Vrijeme = virtualnoVrijeme;
        caretaker.spremi(originator.CreateMemento(naziv.Substring(1, naziv.Length - 2)));
    }

    private static void VratiStanje(string naziv)
    {
        Console.WriteLine(virtualnoVrijeme);
        originator.SetMemento(caretaker.dohvati(naziv.Substring(1, naziv.Length - 2)));
        kolekcijaVezova = originator.Vezovi;
        virtualnoVrijeme = originator.Vrijeme;
        Console.WriteLine("Vraceno stanje vezova na vrijeme: "+virtualnoVrijeme);
    }
